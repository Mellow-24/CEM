...
turn/start
  ...
  -> agent/pre-step                   reject | enter(messages)
     ...
     agent/request -> llm/stream -> streamed: assistant/chunk* -> assistant/message
                                buffered completed text -> agent/post-response -> accept: assistant/chunk* -> assistant/message, or discard and retry
...
`turn/*`, `step/*`, `user/message`, `assistant/*`, and `tool/*` are durable session events; the rest are live extension points across three domains. `agent/pre-step`, `agent/request`, `agent/post-response`, `llm/stream`, and the three `tools/*` events are waterfalls, whose listeners must call `next()` to delegate; `agent/turn-stopping` is serial and has no `next()`.

Input reaches the driver through one inbox. Some messages wake it immediately; injected context waits in the inbox until another message does.

`agent/pre-step` decides what the model sees and may request buffered reply delivery. A buffered completed text response reaches `agent/post-response` before any assistant chunk or message is committed; a recovery listener can discard it and retry the same step with a freshly assembled system prompt. Tool calls, streaming delivery, failed output, and max-token output do not use that interception. A rejected or empty first claim still closes a durable turn that spent no step, so the log records the attempt. Each step reads the prompt sections and tool schemas that plugins registered.
