...
英文源文件由系统**生成**，并通过 `pnpm run verify-tool-catalog`（`doc-sync`（文档同步门禁）的一部分）验证新鲜度；本中文文件作为经评审对侧通过双语配对维护。与 Cordis 目录（纯源码 AST 处理）不同，英文生成器会在真实上下文中**启动**每个工具插件并读取 `ctx.tools.schemas()`，因为工具 schema 无法通过静态分析完全确定，例如运行时展开的枚举、拼接的描述、由配置决定的名称以及使用原始 JSON Schema 的 MCP 工具。完整性守卫既检查惯例中的 `packages/*/tool-*` 目录，也检查所有直接向 `ctx.tools` 注册工具的包源码；schema 由运行时提供的包必须显式说明排除理由，因此新工具不会在无人察觉的情况下缺少文档。参见[工具 schema 目录 Agent Note](../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md)。

范围：具有固定产品工具 schema 的已发布包，每个包均使用其**默认**配置启动；但如果某个 Config 字段是**必填项**且没有默认值，生成器就必须作出选择，对应包的说明会记录本页展示的是哪个分支。注册的工具**名称**可以是加载时配置，例如 `tool-subagent` 的 `toolName`，因此部署可能以不同名称或额外名称提供某个包；如果存在随产品发布的别名，对应包的说明会予以记录。运行时生成的动态包工具、MCP 工具和由调用方提供 schema 的工具由其归属包记录，而不会渲染成一份并不存在的固定 schema。`examples/` 中的演示工具（例如 `echo`）不在范围内，这与 Cordis 目录仅涵盖包的范围一致。
...
| `@deepseek-ai/dsh-customer-service-wiki` | `follow_company_wiki_link`、`open_company_wiki_evidence`、`read_company_wiki`、`read_company_wiki_map` | `ctx.tools`、`ctx.systemPrompt`、执行时可用的已发布不可变 Wiki release | `tool/call`、`tool/result` | - | 仅随澳门 LLM Wiki preset 交付。客服会话获得 4 个只读且绑定 release 的导航与证据工具；编译、审核与发布留在包 CLI 中，绝不进入模型工具目录。 |
...
<a id="deepseek-aidsh-customer-service-wiki"></a>

## `@deepseek-ai/dsh-customer-service-wiki`

### `follow_company_wiki_link`

沿同一份不可变公司 Wiki release 中声明的页面链接继续读取。
...
```json
{
  ...
  "properties": {
    "release_id": {
      "type": "string",
      "description": "Release id returned by read_company_wiki_map."
    },
    "page_id": {
      "type": "string",
      "description": "Current published page id."
    },
    "target_id": {
      "type": "string",
      "description": "Target listed in the current page links."
    }
  },
  "required": [
    "release_id",
...
来源：[`packages/preset/customer-service-wiki/src/index.ts`](../packages/preset/customer-service-wiki/src/index.ts)

### `open_company_wiki_evidence`

从已发布 Wiki 页面授权的确切来源区间中打开正向词法匹配证据。
...
```json
{
  ...
  "properties": {
    "release_id": {
      "type": "string",
      "description": "Release id used to read the page."
    },
    "page_id": {
      "type": "string",
      "description": "Published page authorizing the evidence spans."
    },
    "query": {
      "type": "string",
      "description": "Small factual point that needs source evidence."
    }
  },
  "required": [
    "release_id",
...
从 `read_company_wiki_map` 返回的确切已发布 release 中读取一个页面。
...
```json
{
  ...
  "properties": {
    ...
    "page_id": {
      "type": "string",
      "description": "Page id listed by read_company_wiki_map."
    }
  },
  "required": [
    "release_id",
...
### `read_company_wiki_map`

选择页面前，读取当前已发布的公司 Wiki release 与导航地图。
...
仅随澳门 LLM Wiki preset 交付。客服会话获得 4 个只读且绑定 release 的导航与证据工具；编译、审核与发布留在包 CLI 中，绝不进入模型工具目录。
