...
`agent/post-response` 只在已缓冲的完整文本回复提交其 chunk 和 message 前运行。处理它的 listener 返回 `{ kind: 'retry' }` 且不调用 `next()`，即可丢弃该回复，并为相同请求重新组装系统提示词；其他 listener 会委托或返回 `{ kind: 'accept' }`。流式、工具调用、失败、中止和达到 token 上限的输出会绕过该 waterfall。
