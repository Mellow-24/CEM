...
type PreStepDecision =
  ...
  | {
    ...
    /** Hold assistant chunks until `agent/post-response` accepts them; omitted streams immediately. */
    responseDelivery?: 'stream' | 'buffered'
  }
```
...
`agent/post-response` runs only for a buffered, completed text response before its chunks and message are committed. A handling listener returns `{ kind: 'retry' }` without calling `next()` to discard that response and reassemble the system prompt for the same request; otherwise it delegates or returns `{ kind: 'accept' }`. Streamed, tool-call, failed, aborted, and max-token output bypass this waterfall.

```ts type-equiv
/** Action returned after a buffered, completed model response. */
type PostResponseDecision = { kind: 'accept' } | { kind: 'retry' }
```
...
Source: [`packages/core/agent/src/runtime-types.ts:176`](../../packages/core/agent/src/runtime-types.ts)
...
<a id="agentpost-response--waterfall"></a>

#### `agent/post-response` — waterfall

Accept a buffered completed response or discard it and retry the same request. A listener that returns `{ kind: 'retry' }` owns recovery and does not call `next()`; all other listeners call `next()` to delegate. The retry reassembles the system prompt but leaves the durable user and tool history unchanged. This event is never dispatched for a streamed, max-token, failed, aborted, or tool-call response.

```ts cordis-catalog
/**
 * Accept a buffered completed response or discard it and retry the same
 * request. A listener that returns `{ kind: 'retry' }` owns recovery and
 * does not call `next()`; all other listeners call `next()` to delegate.
 * The retry reassembles the system prompt but leaves the durable user and
 * tool history unchanged. This event is never dispatched for a streamed,
 * max-token, failed, aborted, or tool-call response.
 * @param payload.agent - the agent that received the candidate response.
 * @param payload.turn - the open turn containing the request.
 * @param payload.step - the open step containing the request.
 * @param payload.message - completed assistant message not yet committed to the session log.
 * @param payload.signal - the current turn's explicit abort signal.
 * Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent.
 * @mode waterfall
 */
'agent/post-response'(this: Scoped<Agent>, payload: { agent: Agent; turn: number; step: number; message: AssistantMessage; signal: AbortSignal }, next: () => Promise<PostResponseDecision>): Promise<PostResponseDecision>
```

Types: [AssistantMessage](session.md) · [Scoped](scope.md)
...
Source: [`packages/core/agent/src/runtime-types.ts:239`](../../packages/core/agent/src/runtime-types.ts)
