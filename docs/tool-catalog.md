...
| `@deepseek-ai/dsh-customer-service-wiki` | `follow_company_wiki_link`, `open_company_wiki_evidence`, `read_company_wiki`, `read_company_wiki_map` | `ctx.tools`, `ctx.systemPrompt`, `a published immutable Wiki release at execution time` | `tool/call`, `tool/result` | - | Shipped only in the Macau LLM Wiki preset. Customer sessions receive four read-only, release-pinned navigation and evidence tools; compilation, review, and publication stay in the package CLI and never enter the model tool catalog. |
...
Follow a page link declared in the same immutable company Wiki release.
...
Source: [`packages/preset/customer-service-wiki/src/index.ts`](../packages/preset/customer-service-wiki/src/index.ts)

### `open_company_wiki_evidence`

Open positive lexical matches from the exact source spans authorized by a published Wiki page.
...
Read one page from the exact published release returned by read_company_wiki_map.
...
Read the current published company Wiki release and navigation map before choosing a page.
