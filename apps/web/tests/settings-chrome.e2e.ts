...
const ADMIN_TRIGGER_ZH = '營運管理'
const ADMIN_TITLE_ZH = '澳門電力智能客服營運管理台'
const ADMIN_TRIGGER_EN = 'Operations'
const ADMIN_TITLE_EN = 'Macau Power Intelligent Customer Service Operations Console'
...
describe('web e2e: settings modal and General preferences', () => {
  ...
  /** Open the Chinese or English console on its retained General settings page. */
  async function openGeneral(
    ...
    await target.getByRole('button', { name: triggerName, exact: true }).click()
    const panel = target.getByRole('region', { name: title })
    await panel.waitFor({ timeout: 10_000 })
    const maintenanceName = generalName === 'General' ? 'Developer maintenance' : '開發維護'
    await panel.locator('summary').filter({ hasText: maintenanceName }).click()
    await panel.getByRole('button', { name: generalName, exact: true }).click()
    await expect.poll(
      () => panel.getByRole('button', { name: generalName, exact: true }).getAttribute('aria-current'),
      { timeout: 5_000 },
    ).toBe('page')
    return panel
  }
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    const trigger = page.getByRole('button', { name: ADMIN_TRIGGER_ZH, exact: true })
    expect(await trigger.getAttribute('aria-haspopup')).toBeNull()
    expect(await trigger.getAttribute('aria-expanded')).toBe('false')
    await trigger.click()
    const dialog = page.getByRole('region', { name: ADMIN_TITLE_ZH })
    await dialog.waitFor({ timeout: 10_000 })
    expect(await trigger.getAttribute('aria-expanded')).toBe('true')
    // The customer-service overview is the default. Existing configuration pages remain available.
    expect(await dialog.getByRole('button', { name: '工作台' }).getAttribute('aria-current')).toBe('page')
    await dialog.getByRole('heading', { name: '從每次對話，發現服務改進機會' }).waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: '智能體與渠道' }).click()
    expect(await dialog.getByRole('button', { name: '智能體與渠道' }).getAttribute('aria-current')).toBe('page')
    await dialog.locator('summary').filter({ hasText: '開發維護' }).click()
    await dialog.getByRole('button', { name: '通用設定' }).click()
    await expect.poll(() => dialog.getByRole('button', { name: '通用設定' }).getAttribute('aria-current'), { timeout: 5_000 }).toBe('page')
    await dialog.getByRole('button', { name: 'Workspace Write' }).waitFor({ timeout: 10_000 })
    await expect.poll(() => dialog.getByText('語言', { exact: true }).count(), { timeout: 5_000 }).toBe(1)
    await expect.poll(() => dialog.getByText('外觀', { exact: true }).count(), { timeout: 5_000 }).toBe(1)
    const openDocument = dialog.getByRole('button', { name: '打開配置文件' })
    ...
    // Golden of the composed Chinese console after the existing General page opens.
    const snapshot = await captureStableAria(page, '[role="region"]', scaffold.workspaceCwd)
    ...
    expect(await dialog.getByRole('button', { name: '工作台' }).getAttribute('aria-current')).toBeNull()
    ...
    await dialog.getByRole('heading', { name: '插件', exact: true }).first().waitFor({ timeout: 10_000 })
    ...
    await expect.poll(() => page.getByRole('region', { name: ADMIN_TITLE_ZH }).count(), { timeout: 5_000 }).toBe(0)
    expect(await trigger.getAttribute('aria-expanded')).toBe('false')
    // Close path 2: the header close button (focus lands there on open).
    await trigger.click()
    await page.getByRole('region', { name: ADMIN_TITLE_ZH }).getByRole('button', { name: '關閉' }).click()
    await expect.poll(() => page.getByRole('region', { name: ADMIN_TITLE_ZH }).count(), { timeout: 5_000 }).toBe(0)
  ...
  it('stores Permission as the default for future sessions without changing an existing session', async () => {
    ...
    const dialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    ...
    const confirmation = page.getByRole('dialog', { name: '確認啟用 Full access？' })
    const enable = confirmation.getByRole('button', { name: '啟用 Full access' })
  ...
  it('uses the persisted dark preference while plugins are still loading', async () => {
    ...
    const initialDialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    ...
    const restoredDialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    const systemCube = restoredDialog.getByRole('button', { name: '跟隨系統' })
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const systemDialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    const systemCube = systemDialog.getByRole('button', { name: '跟隨系統' })
    await systemCube.click()
    await expect.poll(() => systemCube.getAttribute('aria-pressed'), { timeout: 5_000 }).toBe('true')
    ...
    await systemDialog.getByRole('button', { name: '淺色' }).click()
    await expect.poll(async () => (await readState()).attr, { timeout: 5_000 }).toBe(false)
    expectThemeColorSynchronized(await readState())
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    const dialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    await dialog.getByRole('button', { name: '排隊發送' }).click()
    await page.getByRole('menuitem', { name: '插話發送' }).click()
    await dialog.getByRole('button', { name: '插話發送' }).waitFor({ timeout: 10_000 })
    ...
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    acknowledgeReloadConnectionLoss(tripwire, warningStart)
    const reloaded = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    await reloaded.getByRole('button', { name: '插話發送' }).waitFor({ timeout: 10_000 })
    ...
    try {
      ...
      await secondPage.goto(second.baseUrl, { waitUntil: 'load' })
      await secondPage.waitForSelector('[class*="frame"]', { timeout: 30_000 })
      const secondDialog = await openGeneral(secondPage, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
      await secondDialog.getByRole('button', { name: '插話發送' }).waitFor({ timeout: 10_000 })
    ...
    await reloaded.getByRole('button', { name: '插話發送' }).click()
    await page.getByRole('menuitem', { name: '排隊發送' }).click()
    await reloaded.getByRole('button', { name: '排隊發送' }).waitFor({ timeout: 10_000 })
    expect(await page.evaluate(() => localStorage.getItem('dsh.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/ui-conversation:\n\s+busyEnter: queue/)
    await page.keyboard.press('Escape')
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    const zhDialog = await openGeneral(page, ADMIN_TRIGGER_ZH, ADMIN_TITLE_ZH, '通用設定')
    // The Language selector pill shows the active locale's own name.
    const selector = zhDialog.getByRole('button', { name: '繁體中文' })
    ...
    const enDialog = page.getByRole('region', { name: ADMIN_TITLE_EN })
    await enDialog.waitFor({ timeout: 10_000 })
    await enDialog.getByRole('button', { name: 'General' }).click()
    ...
    // other specs' 設定-anchored selectors + goldens) see the default again.
    const warningStart = tripwire.warnings.length
    await page.reload({ waitUntil: 'load' })
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    acknowledgeReloadConnectionLoss(tripwire, warningStart)
    const enTrigger = page.getByRole('button', { name: ADMIN_TRIGGER_EN })
    ...
    try {
      ...
      await secondPage.goto(second.baseUrl, { waitUntil: 'load' })
      await secondPage.waitForSelector('[class*="frame"]', { timeout: 30_000 })
      const secondDialog = await openGeneral(secondPage, ADMIN_TRIGGER_EN, ADMIN_TITLE_EN, 'General')
      await secondDialog.getByRole('button', { name: 'English' }).waitFor({ timeout: 10_000 })
    ...
    await enTrigger.waitFor({ timeout: 10_000 })
    const restoredEnDialog = await openGeneral(page, ADMIN_TRIGGER_EN, ADMIN_TITLE_EN, 'General')
    await restoredEnDialog.getByRole('button', { name: 'English' }).click()
    await page.getByRole('menuitem', { name: '繁體中文' }).click()
    await page.getByRole('region', { name: ADMIN_TITLE_ZH }).waitFor({ timeout: 10_000 })
    expect(await page.evaluate(() => localStorage.getItem('dsh.locale'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/locale:\n\s+preference: zh/)
    await page.keyboard.press('Escape')
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      const dialog = await openGeneral(enPage, ADMIN_TRIGGER_EN, ADMIN_TITLE_EN, 'General')
