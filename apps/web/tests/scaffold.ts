...
// replay (default, keyless: disables the shipped model-provider rows and
...
// built dist; ordinary keyless modes disable shipped model adapters and fill the open
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: '深绎未来智能服务平台现处于演示与优化阶段。我们会持续完善模型服务、知识管理与会话运营能力，并欢迎业务团队提出真实场景中的改进建议。\n\n平台致力于为行业客户提供可信、易用、可持续演进的智能服务体验。',
    continueLabel: '继续',
  },
} as const
...
// Replay publishes the provider catalogs the gateway and shipped presets route to (providers
// mode, never catch-all: with shipped model adapters disabled no adapter exists, so a
...
const REPLAY_PROVIDERS = [{
  ...
  name: '深绎未来模型服务',
  models: [{ id: 'deepseek-v4-flash', name: '深绎·迅捷', contextWindow: 128_000 }],
}, {
  id: 'qwen-dashscope',
  name: '外部模型服务',
  models: [
    { id: 'qwen3.7-plus', name: '客服质检模型', contextWindow: 1_048_576 },
    { id: 'qwen3.7-flash', name: '客服回答模型', contextWindow: 1_048_576 },
  ],
}]
...
export interface LaunchOptions {
   ...
   * fails loud with NO_ADAPTER (shipped model adapters are disabled and no replay row
   * mounts).
   */
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // The management console reads the shipped Macau corpus while each test
    // keeps its upload staging directory inside its disposable workspace.
    // `launchWebScaffold()` deliberately changes the process cwd to that
    // workspace, so the production relative defaults cannot identify the
    // repository-owned source artifacts here.
    {
      id: 'customer-service-admin',
      config: {
        rag: {
          sourceDirectory: join(REPO_ROOT, 'knowledge/macau-customer-service'),
          sourceManifestPath: join(REPO_ROOT, 'knowledge/macau-customer-service/source-manifest.json'),
          indexPath: join(REPO_ROOT, '.dsh/macau-customer-service.index.json'),
          embeddingBaseURL: 'http://127.0.0.1:1234/v1',
          embeddingModel: 'text-embedding-qwen3-embedding-0.6b',
          rerankerURL: 'http://127.0.0.1:1234/v1/rerank',
        ...
        wiki: {
          sourceDirectory: join(REPO_ROOT, 'knowledge/macau-customer-service-wiki/sources'),
          releaseDirectory: join(REPO_ROOT, 'knowledge/macau-customer-service-wiki/release'),
          navigationMaxChars: 12000,
          evidenceChunkChars: 1200,
          evidenceChunkOverlapChars: 160,
          resultCount: 4,
          maxResultChars: 1400,
          queryMaxChars: 1000,
        },
        badCasesPath: join(REPO_ROOT, 'knowledge/macau-customer-service-eval/bad-cases.jsonl'),
        stagingDirectory: join(workspaceCwd, '.dsh/macau-customer-service-admin/staging'),
        quality: {
          directory: join(workspaceCwd, '.dsh/macau-customer-service-admin/quality'),
          provider: 'quality-test', model: 'quality-test', timeoutMs: 10000, maxEvents: 60000,
          maxInputBytes: 180000, maxOutputTokens: 12000, maxRecordBytes: 2097152, maxRuns: 500, maxTurns: 12,
        ...
        limits: {
          ...
          maxWikiPages: 5000,
          maxBadCases: 1000,
          maxBadCasesBytes: 4194304,
          maxIndexBytes: 67108864,
          maxStagedDocumentBytes: 2097152,
        },
      },
    },
    ...
    ...mode === 'record'
      ? []
      : [
        ...options.deepSeekMissingCredential === true ? [] : [{ id: 'llm-deepseek', disabled: true }],
  ...
  try {
    ...
    // disable shipped model adapters; the first-run lane keeps DeepSeek mounted but has no
...
function normalizeAria(snapshot: string, workspaceCwd: string): string {
  ...
  return snapshot
    ...
    .replace(/(?<!\d)\d{2}\/\d{2} \d{2}:\d{2}(?!\d)/g, '{{clock}}')
