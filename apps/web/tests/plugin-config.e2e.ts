...
const ADMIN_TRIGGER_ZH = '运营管理'
const ADMIN_TITLE_ZH = '澳门电力智能客服运营管理台'
...
describe('web e2e: plugin configuration section', () => {
  ...
  async function openPlugins() {
    if (await page.getByRole('dialog', { name: ADMIN_TITLE_ZH }).count() > 0) {
      await page.keyboard.press('Escape')
      await expect.poll(() => page.getByRole('dialog', { name: ADMIN_TITLE_ZH }).count(), { timeout: 5_000 }).toBe(0)
    }
    await page.getByRole('button', { name: ADMIN_TRIGGER_ZH, exact: true }).click()
    const dialog = page.getByRole('dialog', { name: ADMIN_TITLE_ZH })
