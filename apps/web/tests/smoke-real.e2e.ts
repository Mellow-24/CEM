...
const UI_PLUGIN_DIRS = [
  ...
  'ui-model-selection', 'ui-response-language', 'ui-user-questions', 'ui-trajectory', '../session-query/session-log-export',
]
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () => document.title !== '深绎未来' && document.title.endsWith(' — 深绎未来'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — 深绎未来`,
      durableTitle,
      { timeout: 15_000 },
    )
