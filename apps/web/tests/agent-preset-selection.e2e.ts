...
const LANGUAGE_MENU_EXPECTED = join(SNAPSHOT_DIR, 'language-menu.expected.md')
...
describe('web e2e: agent-preset selection', () => {
  ...
  it('shows the reply-language selector only while the blank session uses a supporting preset', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-response-language'))
    await page.getByRole('button', { name: 'Standard mode' }).click()
    await page.getByRole('menuitem', { name: /^Macau customer service/ }).first().click()
    await expect.poll(() => livePreset(scaffold.baseUrl), { timeout: 15_000 })
      .toBe('macau-customer-service')

    const language = page.getByRole('button', { name: 'Response language: Auto' })
    await language.waitFor({ timeout: 15_000 })
    await language.click()
    const snapshot = await captureStableAria(page, '[role="menu"]', scaffold.workspaceCwd)
    await compareOrRefreshGolden(LANGUAGE_MENU_EXPECTED, snapshot, MODE)
    expect(snapshot).toContain('Auto')
    expect(snapshot).toContain('澳門粵語')
    expect(snapshot).toContain('香港粵語')
    expect(snapshot).toContain('English')
    expect(snapshot).toContain('Português')

    await page.getByRole('menuitem', { name: 'English' }).click()
    await page.getByRole('button', { name: 'Response language: English' }).waitFor({ timeout: 15_000 })
    const live = scaffold.ctx.agents.list().find(agent => String(agent.id) !== SEED_ID)
    expect(live?.session.events.findLast(event => event.type === 'response-language/preference')?.data)
      .toEqual({ value: 'en' })

    await page.getByRole('button', { name: 'Macau customer service' }).click()
    await page.getByRole('menuitem', { name: /^Minimal mode/ }).click()
    await expect.poll(() => livePreset(scaffold.baseUrl), { timeout: 15_000 }).toBe('minimal')
    await expect.poll(() => page.getByRole('button', { name: /^Response language:/ }).count())
      .toBe(0)
  }, 90_000)
