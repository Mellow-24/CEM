...
import {
  bootWebPresetHarness, CONFIG_DIR, EXAMPLES_INSTALL_ANCHOR, toolNames,
...
beforeAll(async () => {
  ...
  ctx = await bootWebPresetHarness(settingsFile)
}, 120_000)
...
afterAll(async () => {
  try {
    ...
    if (previousDashScopeKey === undefined) Reflect.deleteProperty(process.env, 'DASHSCOPE_API_KEY')
...
describe('the shipped Web composition', () => {
  ...
  it('supplies all seven shipped presets, and only those, from the system root', async () => {
    const listed = await ctx.agentPresets.list()

    expect(listed.map(preset => preset.id)).toEqual([
      'standard', 'code', 'minimal', 'cordis', 'macau-customer-service', 'macau-customer-service-wiki',
      'hk-feng-shui',
    ])
  ...
  it('composes the Hong Kong feng shui persona without model-facing tools', async () => {
    const handle = await ctx.agents.create({
      sessionId: SessionId('preset-hk-feng-shui'),
    ...
    try {
      ...
      expect(assembly.sections.find(section => section.name === 'deployment:persona')?.text)
        .toContain('你是「香港風水顧問」，一個虛擬 AI 風水文化角色。')
      expect(assembly.sections.find(section => section.name === 'response-language:policy')?.text)
        .toContain('Reply in natural Hong Kong Cantonese written with Traditional Chinese characters.')
      expect(() => ctx.speechSynthesis.profile(handle.agent, 'hk-feng-shui'))
  ...
  it('isolates the two customer-service compositions and disposes their standing registrations', async () => {
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'dsh-customer-presets-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
    const customerCtx = await bootWebPresetHarness(settingsFile)
    const toolRegistry = customerCtx.tools
    const handles: AgentHandle[] = []
    const composedAgents: Agent[] = []
    try {
      const rag = await customerCtx.agents.create({
        sessionId: SessionId('preset-customer-rag'),
        setup: agentCtx => customerCtx.agentPresets.mount(agentCtx, 'macau-customer-service').then(() => undefined),
      })
      handles.push(rag)
      composedAgents.push(rag.agent)
      const wiki = await customerCtx.agents.create({
        sessionId: SessionId('preset-customer-wiki'),
        setup: agentCtx => customerCtx.agentPresets.mount(agentCtx, 'macau-customer-service-wiki').then(() => undefined),
      })
      handles.push(wiki)
      composedAgents.push(wiki.agent)

      expect(toolNames(customerCtx)).toEqual([])
      expect(toolNames(customerCtx, rag.agent)).toEqual([])
      expect(toolNames(customerCtx, wiki.agent)).toEqual([
        'follow_company_wiki_link', 'open_company_wiki_evidence',
        'read_company_wiki', 'read_company_wiki_map',
      ])

      const ragSections = (await customerCtx.systemPrompt.assemble({ scope: rag.agent })).sections
        .map(section => section.name)
      const wikiSections = (await customerCtx.systemPrompt.assemble({ scope: wiki.agent })).sections
        .map(section => section.name)
      expect(ragSections).toEqual(expect.arrayContaining(['deployment:persona', 'knowledge:approved-company-evidence']))
      expect(ragSections).not.toContain('tool:company_wiki')
      expect(wikiSections).toEqual(expect.arrayContaining(['deployment:persona', 'tool:company_wiki']))
      expect(wikiSections).not.toContain('knowledge:approved-company-evidence')

    } finally {
      try {
        await Promise.all(handles.map(handle => handle.dispose()))
      } finally {
        await customerCtx.fiber.dispose()
      }
    }

    expect(composedAgents.map(agent => toolRegistry.schemas(agent))).toEqual([[], []])
  })
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    ...
    const handle = await ctx.agents.create({
      // Unique per retry: the shared runtime keeps the Session object after an
      // agent handle is disposed, so a retry must not adopt the prior attempt.
      sessionId: SessionId(`preset-skills-standard-${randomUUID()}`),
      // The loader tool derives local skill roots from the owning session, not
      // from the list() call above; pin the same project instead of letting a
      // cwd-less fixture fall back to the machine's real Harness home.
      meta: { cwd: proj },
      setup: agentCtx => ctx.agentPresets.mount(agentCtx, 'standard').then(() => undefined),
    })
    try {
      ...
      expect(loaded.isError, JSON.stringify(loaded)).toBe(false)
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    ...
    productCtx = await bootWebPresetHarness(settingsFile, [
      { insert: [
...
describe('a launcher that configures no writable root', () => {
  ...
  beforeAll(async () => {
    ...
    derivedCtx = await bootWebPresetHarness(settingsFile, [{
      id: 'agent-presets',
      config: {
        default: 'standard',
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(settingsFile, '{}\n')
    authorCtx = await bootWebPresetHarness(settingsFile, [{
      id: 'agent-presets',
      config: {
        default: 'standard',
        roots: [
