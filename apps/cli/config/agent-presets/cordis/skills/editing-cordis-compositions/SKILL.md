...
**Never edit, delete, or overwrite a preset that ships with the deployment** — the `agent-presets` directory beside the deployment's own config, which supplies `standard`, `code`, `minimal`, `cordis`, `macau-customer-service`, `macau-customer-service-wiki`, and `hk-feng-shui`. Never escalate the sandbox to reach it, even when a change there looks quicker. An upgrade overwrites that install, and corrupting `cordis` disables preset authoring itself. Reading a shipped composition is the intended way to start; writing to one is not, and neither is editing the host composition to work around a preset limitation.
...
**Agent preset.** What agents on one preset contribute to those registries: tool plugins, persona and prompt sections, and compaction policy. The roster mounts one standing plugin generation per preset and parents each joining agent's scope to it; plugin-owned mutable state must still key itself by Session or Agent. A generation stays alive while the process runs, including after its composition file changes for later sessions.

**A service with a consumer outside the agent plane cannot move into a preset.** `subagents` is the worked example: the registry answers cross-session queries for the host api-proxy, so a preset-private copy starves that host row — it waits forever for the process-level service nothing provides. The preset contributes the delegation *tools*; the registry and its backends stay host-side.
...
**A row that publishes a service may not sit loose in a preset.** Registering a service without an isolate realm puts it in the process-global realm, where the host can resolve one preset's instance and another preset generation can collide with it. The mount rejects the first leaked registration rather than letting either failure surface later.
...
`true` means a realm private to this standing preset generation. Every agent joined to that generation sees the same service instance, so the service must key per-session state by Session or Agent. A string label instead joins otherwise separate subtrees into one shared realm; `provide()` still throws on the second registration under that symbol, so a label does not pool instances and is not what a preset needs.
