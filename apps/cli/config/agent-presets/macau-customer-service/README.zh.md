...
RAG 知识库将已审核事实与可选的「客服口语示例（澳门粤语）」分开记录，并一同索引已审核的普通话、繁体中文和粤语口语相似问法。客服保留「回答」中的事实，不会把知识原文当成播报稿照读。解析为 `en` 时必须完全使用英文，并忽略粤语表达示例；解析为 `yue-Hant-MO` 时才用示例校准自然澳门客服口吻。这项处理不增加模型请求或生成后校验步骤。
