...
The RAG corpus separates approved facts from optional `客服口語示例（澳門粵語）` phrasing. Retrieval indexes reviewed Mandarin, Traditional Chinese, and colloquial Cantonese aliases together. The agent preserves the approved facts and never treats evidence text as a script to read verbatim. An `en` resolution requires an entirely English answer and ignores Cantonese wording examples; `yue-Hant-MO` uses the examples only to render natural Macau customer-service Cantonese. This adds no model request or post-generation verification stage.
