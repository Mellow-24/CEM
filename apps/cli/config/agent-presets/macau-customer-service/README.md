...
Auto greets new calls in Cantonese, then follows decisive question language. Ambiguous short input carries the previous language and falls back to Cantonese without context. An explicit language selection takes priority. Detection uses existing transcript text and cannot reliably recover dialect erased by transcription.
## Start
