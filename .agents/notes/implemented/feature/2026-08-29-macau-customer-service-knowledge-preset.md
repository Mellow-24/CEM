...
The source root and manifest must exist before the preset can mount. Every direct customer message incurs retrieval before its single answer request. First search after an approved corpus or index-identity change performs embedding before answering; later searches reuse the cache after revalidating source bytes and derived chunks. Operators must calibrate both relevance thresholds with approved positive and out-of-domain questions for their selected models.
