...
Two adjacent gaps compounded this. pi-ai decides the reasoning *wire dialect* (`compat.thinkingFormat`, `compat.supportsReasoningEffort`) and whether a reasoning model receives its system instruction as OpenAI's newer `developer` role by recognizing the endpoint URL. A private gateway's URL says nothing: a DeepSeek-dialect gateway could receive the OpenAI dialect, while an OpenAI-compatible endpoint such as DashScope could reject the inferred `developer` role. And the only way to touch one catalog model was the `models` list, which *replaces* the served catalog: narrowing `gpt-5`'s levels meant restating all thirty-eight openai models or silently dropping thirty-seven.
...
- **Keep URL-derived `developer`-role detection.** A standard-looking OpenAI-compatible URL does not prove that the endpoint accepts the newer role, and the resulting rejection occurs only after the request reaches the provider. An explicit boolean keeps the system-message role aligned with the endpoint.
...
- A reasoning-capable model can retain the `system` role on compatible endpoints that reject `developer`, without disabling reasoning capability or changing the selected reasoning effort.
