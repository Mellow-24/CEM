...
Booting has a cost the schema AST pass did not: there is no schema declaration set to enumerate, so a new tool package could simply be forgotten. A **completeness guard** restores the guarantee from two independent inventories: conventional `packages/*/tool-*` leaves and a TypeScript AST scan for direct `<context>.tools.register(...)` calls anywhere under `packages/*/*/src`. Every discovered package must have a `TOOL_PACKAGES` boot recipe or an explicit reason why its runtime-supplied schema cannot be harvested once. The AST scan discovers package ownership only; the schema still comes exclusively from booting the plugin and reading the registry. An omission fails the generator and therefore `doc-sync`.

### A hand-maintained boot manifest is the irreducible policy

The filesystem and registration-call scan discover the tool-package inventory, and the completeness guard rejects omissions, empty exclusion reasons, stale exclusions, and a package listed as both harvestable and excluded. `TOOL_PACKAGES` still owns an explicit boot recipe for each fixed-schema package because required Service Providers and config are policy, not facts that can be inferred safely from layout or injection names.

### Scope

Shipped packages with fixed product tool schemas, each booted with its default config, including conventional `tool-*` packages and fixed-schema registrants elsewhere such as the Macau customer-service packages. Model-authored dynamic-package tools, connected MCP server tools, and per-run structured-output tools remain outside the rendered catalog because no single package-owned schema represents them; their owning packages are explicit, reasoned exclusions. Example-only tools are excluded.
...
`scripts/gen-tool-catalog.spec.ts` pins discovery of non-`tool-*` registrants, explicit reasons for runtime schemas, the conventional directory fallback, and rejection of stale or overlapping exclusions. The ordinary `verify-tool-catalog` run exercises the complete repository inventory before it boots and harvests every fixed-schema manifest entry.

## Consequences

- The catalog cannot drift: a tool schema change the committed file doesn't reflect fails `verify-tool-catalog` in `doc-sync` and CI. A new conventional tool package or direct tool registrant not added to the manifest fails the completeness guard outright, even when its directory does not start with `tool-`.
