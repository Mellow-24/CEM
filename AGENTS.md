...
packages/    @deepseek-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  speech/      transcription/synthesis, HTTP/Web adapters
...
pnpm run test:e2e       # live-provider tests; key-gated
...
Run pre-push checks via [dsh-pre-push-checks](.agents/skills/dsh-pre-push-checks/SKILL.md); report commands. Validate after `gh stack sync`; do not merge before checks pass.
...
Provider-keyed real-API tests read root `.env` or the environment; demos use `DEEPSEEK_API_KEY` and optional `DEEPSEEK_BASE_URL`. cordis.yml permits `!!js` (never `!js`) only in plugin `config` and entry `disabled`; metadata stays literal, so conditionals use overlays ([primer](docs/cordis-primer.md#loader-configuration)). Never commit credentials. Keyless CI skips real-provider cases; [testing.md](docs/testing.md) owns policy.
