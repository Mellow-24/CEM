...
import * as CustomerServiceWiki from '@deepseek-ai/dsh-customer-service-wiki'
...
/** A tool-registering package whose schemas exist only at runtime and therefore cannot be harvested once. */
interface ToolRegistrationExclusion {
  /** The `packages/<group>/<dir>` leaf name discovered from its registration source. */
  dir: string
  /** Why one fixed-schema catalog entry cannot represent the package. */
  reason: string
}

/**
 * Explicit exceptions to fixed-schema harvesting. Each package still enters the
 * completeness inventory; it is excluded only because another runtime supplies
 * the schema or a caller supplies a per-run schema.
 */
const TOOL_REGISTRATION_EXCLUSIONS: readonly ToolRegistrationExclusion[] = [
  {
    dir: 'cordis-host-runner',
    reason: 'registers model-authored dynamic-package tools whose names and schemas do not exist before the package runs',
  },
  {
    dir: 'mcp-client',
    reason: 'registers the connected MCP server\'s live tool inventory rather than one package-owned schema set',
  },
  {
    dir: 'subagent-in-process-driver',
    reason: 'registers one child-scoped structured_output tool from the caller-supplied result schema',
  },
]

/**
 * The boot manifest for every package with a fixed product tool schema.
 * Ordered by package name (the render order); the completeness guard checks it
 * against conventional `tool-*` packages and actual registry call sites.
 */
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-customer-service-wiki',
    dir: 'customer-service-wiki',
    source: 'packages/preset/customer-service-wiki/src/index.ts',
    requires: ['ctx.tools', 'ctx.systemPrompt', 'a published immutable Wiki release at execution time'],
    writes: ['tool/call', 'tool/result'],
    async mount(ctx) {
      await ctx.plugin(CustomerServiceWiki, {
        releaseDirectory: resolve(root, 'knowledge/macau-customer-service-wiki/release'),
        navigationMaxChars: 12_000,
        evidenceChunkChars: 1_200,
        evidenceChunkOverlapChars: 160,
        resultCount: 4,
        maxResultChars: 1_400,
        queryMaxChars: 1_000,
        timeoutMs: 60_000,
      })
    },
    note:
      'Shipped only in the Macau LLM Wiki preset. Customer sessions receive four read-only, release-pinned navigation and evidence tools; compilation, review, and publication stay in the package CLI and never enter the model tool catalog.',
  },
  {
...
/**
 * Whether one source file directly calls a context's tool registry.
 * @param filename - path used for parser mode and diagnostics.
 * @param source - TypeScript source text.
 * @returns whether the AST contains `<context>.tools.register(...)`.
 */
function registersModelTool(filename: string, source: string): boolean {
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)
      ...
      if (ts.isPropertyAccessExpression(registry) && registry.name.text === 'tools') {
        found = true
        return
      }
    }
...
/** Discover package leaves that directly register into a context's tool registry. */
function discoverToolRegistrantDirs(scanRoot: string): string[] {
  const dirs = new Set<string>()
  for (const filename of globSync('packages/*/*/src/**/*.{ts,tsx}', { cwd: scanRoot })) {
    const match = /^packages\/[^/]+\/([^/]+)\/src\//.exec(filename)
    /* v8 ignore next -- the glob has exactly the package/source prefix above. */
    if (match?.[1] === undefined) continue
    const source = readFileSync(resolve(scanRoot, filename), 'utf8')
    if (registersModelTool(filename, source)) dirs.add(match[1])
  }
  return [...dirs].sort()
}

/**
 * Assert the boot manifest covers every conventional tool package and every
 * package that directly registers model tools. Runtime-schema packages require
 * an explicit, reasoned exclusion rather than disappearing from the inventory.
 * Exported for direct negative tests.
 *
 * `scanRoot` defaults to the repo root; a test may point it at a fixture tree.
 * @param packages - fixed-schema packages with boot recipes.
 * @param scanRoot - repository or fixture root to inspect.
 * @param exclusions - explicitly non-harvestable runtime-schema packages.
 */
export function assertManifestComplete(
  packages: ToolPackage[] = TOOL_PACKAGES,
  scanRoot: string = root,
  exclusions: readonly ToolRegistrationExclusion[] = TOOL_REGISTRATION_EXCLUSIONS,
): void {
  const conventional = globSync('packages/*/tool-*', { cwd: scanRoot }).map(path => basename(path))
  const registrants = discoverToolRegistrantDirs(scanRoot)
  const onDisk = [...new Set([...conventional, ...registrants])].sort()
  const listed = new Set(packages.map(p => p.dir))
  const excluded = new Set(exclusions.map(entry => entry.dir))
  const overlap = [...excluded].filter(dir => listed.has(dir))
  if (overlap.length > 0) {
    throw new Error(`gen-tool-catalog: package(s) cannot be both catalogued and excluded: ${overlap.join(', ')}`)
  }
  const invalidExclusion = exclusions.find(entry => entry.reason.trim().length === 0)
  if (invalidExclusion !== undefined) {
    throw new Error(`gen-tool-catalog: excluded package ${invalidExclusion.dir} must state why its schemas cannot be harvested`)
  }
  const staleExclusions = exclusions.filter(entry => !registrants.includes(entry.dir)).map(entry => entry.dir)
  if (staleExclusions.length > 0) {
    throw new Error(`gen-tool-catalog: exclusion(s) no longer register model tools: ${staleExclusions.join(', ')}`)
  }
  const missing = onDisk.filter(dir => !listed.has(dir) && !excluded.has(dir))
  if (missing.length > 0) {
    throw new Error(
      `gen-tool-catalog: ${missing.length} tool-registering package(s) not in the manifest: ${missing.join(', ')}. `
      + 'Add each fixed schema to TOOL_PACKAGES, or record why a runtime-supplied schema cannot be harvested.',
    )
  }
}
...
export function render(catalog: ToolCatalog): string {
  const lines: string[] = [
    ...
    '',
    'This file is GENERATED and verified fresh by `pnpm run verify-tool-catalog` (part of `doc-sync`) — do not edit it by hand. Unlike the cordis catalog (a pure source-AST pass), this generator BOOTS each tool plugin on a real context and reads `ctx.tools.schemas()`, because a tool schema is not statically knowable (runtime-spread enums, concatenated descriptions, config-driven names, raw-JSON-Schema MCP tools). A completeness guard checks both conventional `packages/*/tool-*` leaves and every package source that directly registers into `ctx.tools`; a package with runtime-supplied schemas requires an explicit reasoned exclusion, so a new tool cannot be silently undocumented. See [the tool-schema-catalog Agent Note](../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.md).',
    '',
    'Scope: shipped packages with fixed product tool schemas, each booted with its DEFAULT config, except where a Config field is REQUIRED with no default — there the generator must choose, and the per-package note records which branch this page shows. The registered tool NAME can be a load-time config (e.g. `tool-subagent`\'s `toolName`), so a deployment may expose a package under a different or additional name — a per-package note records those shipped aliases where they exist. Runtime-generated dynamic-package, MCP, and caller-schema tools are documented by their owning packages rather than rendered as one fictitious fixed schema. The `examples/` demo tools (e.g. `echo`) are excluded, matching the cordis catalog\'s packages-only scope.',
    '',
    '## Tool Package Map',
    '',
