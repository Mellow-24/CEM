...
describe('agent loop', () => {
  ...
  it('does not dispatch post-response for a buffered tool-call step', async () => {
    const adapter = new MockAdapter([
      toolCallResponse('c1', 'echo', { text: 'ping' }, 'calling echo'),
      textResponse('done'),
    ])
    ...
    const agent = ctx.agentLoop.create(SessionId('buffered-tool-call'), { provider: 'mock', model: 'mock' })
