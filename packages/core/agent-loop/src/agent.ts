...
export class ReactLoopAgent implements Agent {
  ...
  private async preStep(target: InboxTarget, position: { turn: number; step: number }): Promise<PreparedStep> {
    ...
    return decision.kind === 'reject'
      ? decision
      : { ...decision, assembly, responseDelivery: decision.responseDelivery ?? 'stream' }
  }
    ...
    try {
      while (true) {
        ...
        try {
          ...
          const stepEnd = await this.step(decision.assembly, decision.responseDelivery)
  ...
  private async step(
    initialAssembly: PromptAssembly,
    responseDelivery: 'stream' | 'buffered',
    ...
    while (true) {
      ...
      if (responseDelivery === 'buffered' && finish.kind !== 'max-tokens' && toolCalls.length === 0) {
        const action = await this.dispatch.waterfall(
          'agent/post-response',
          { turn, step, message, signal },
          (): Promise<PostResponseDecision> => Promise.resolve({ kind: 'accept' }),
        )
        signal.throwIfAborted()
        if (action.kind === 'retry') {
