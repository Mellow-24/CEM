...
在不定义工具的情况下添加模型可见请求上下文的产品 plugin。`agent-instructions` 包含在默认 `dsh-agent-spine-demo` 组合包中，可通过组合包配置禁用；`response-language` 由随附澳门客服预设组合；`time-context`、`tmux-context` 和 `session-reference` 需主动启用。

| 包 | 职责 | ctx key |
|---|---|---|
| [`session-reference/`](session-reference/README.md) | 其他会话的有界快照 | `ctx.sessionReferenceResolver` |
| [`response-language/`](response-language/README.md) | 记录在日志中的回复语言选择与请求解析 | — |
