...
`response-language/resolved` records the preference, fixed language, basis, turn, and step used by an accepted request. A detected result also records the direct human message id and confidence. The plugin observes `agent/inbox/claimed` before prompt assembly, but only direct `source.kind === 'user'` text participates. Fixed selection wins without detection; Auto uses a decisive allowed local detection. Short English social expressions such as `thank u`, `OK, thanks`, and `bye bye` carry the previous result rather than selecting English, including under a restricted policy, and use `fallbackLanguage` when the session has no prior result. Internal continuations also carry the previous result. With the default unrestricted list, other ambiguous direct input carries the previous result; a restricted list sends it to the fallback. Tool results, evidence, plugin context, and assistant messages never vote.
...
- **Bounded Auto detector** — short, transliterated, code-switched, or dialect-neutral text can remain ambiguous. Recognized English social continuations carry the previous result under either policy. Other unmatched direct input carries under an unrestricted policy and uses the configured fallback under a restricted `autoDetectedLanguages` policy. The bounded business and social lexicons cannot classify every colloquial expression.
