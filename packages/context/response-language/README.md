...
`fallbackLanguage` is one of `zh-Hans`, `zh-Hant`, `yue-Hant-MO`, `yue-Hant-HK`, `en`, or `pt`; omission defaults to `zh-Hant`. The plugin must be mounted in an agent-preset scope. Its fixed preference vocabulary is `auto` followed by those six languages.
...
- **Bounded Auto detector** — short, transliterated, or code-switched text can remain ambiguous and therefore carries the previous result or uses the configured fallback.
