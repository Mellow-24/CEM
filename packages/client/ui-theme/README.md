...
`src/styles/` holds the sheets imported by the web shell's `base.css`: `base.css`, `design-platform.css`, `shenyi-brand.css`, `scrollbar.css`, `gradient-shadow-text.css`, `shiki.css`, and `cem-mobile.css`. `shenyi-brand.css` follows the platform sheet and remaps semantic aliases for the product identity; `scrollbar.css` follows it and remains the sole consumer of the `--dsw-alias-scrollbar-*` tokens. The CEM palette applies only under `[data-cem-mobile="chat"]` and does not change other roots.
