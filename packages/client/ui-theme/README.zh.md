...
`src/styles/` 包含 web 壳的 `base.css` 导入的样式表：`base.css`、`design-platform.css`、`shenyi-brand.css`、`scrollbar.css`、`gradient-shadow-text.css`、`shiki.css` 与 `cem-mobile.css`。`shenyi-brand.css` 紧随平台样式表，以产品品牌映射语义别名；`scrollbar.css` 排在其后，仍是 `--dsw-alias-scrollbar-*` token 的唯一消费方。澳电调色板仅在 `[data-cem-mobile="chat"]` 内生效，不改变其他根节点。
