...
/** Complete Chinese fallback plus any translated dictionaries shipped by one namespace owner. */
export type DeclaredLocaleDictionaries<N extends keyof LocaleNamespaceMap & string> = {
  zh: LocaleDictOf<N>
} & Partial<Record<Exclude<LocaleId, 'zh'>, LocaleDictOf<N>>>
...
export class LocaleRuntime {
   ...
   * {@link LocaleNamespaceMap} key union. The complete zh fallback is required;
   * other shipped locales may be added per namespace as their product surface
   * is translated. Duplicate (ns, locale) throws (single occupant; a
   ...
   * @param dicts - fallback dictionary plus translated locale dictionaries.
   * @returns disposer removing every locale registered by this call (idempotent).
   */
  register<N extends keyof LocaleNamespaceMap & string>(ns: N, dicts: DeclaredLocaleDictionaries<N>): () => void
  /**
