...
A session may change preset only while it is blank. Once a conversation starts, the host refuses a different preset because that session's history was produced under its current tools. The row therefore is not a live switch: changes apply only to sessions opened afterwards, while running sessions keep their composition.
...
Preset files publish one unlocalized `name` and `description`, which Web uses for every `user` row and unknown `system` row. For the seven shipped ids (`standard`, `code`, `minimal`, `cordis`, `macau-customer-service`, `macau-customer-service-wiki`, and `hk-feng-shui`), Web resolves both fields from its active locale only when the roster marks the row `system`; an identically named `user` preset keeps its file metadata.
...
Deleting removes the preset directory. Sessions already joined to its standing composition keep running; new sessions can no longer resolve it from the roster.
