...
会话只能在仍为空白时更换 preset。对话一旦开始，宿主就会拒绝不同的 preset，因为该会话的历史是在当前工具下产生的。因此本行不是实时切换：更改只对此后开启的会话生效，而运行中的会话保持自身组装。
...
preset 文件提供一套未国际化的 `name` 与 `description`，Web 将其用于所有 `user` 行和未知的 `system` 行。对于 7 个随附 id（`standard`、`code`、`minimal`、`cordis`、`macau-customer-service`、`macau-customer-service-wiki` 与 `hk-feng-shui`），只有名单将该行标记为 `system` 时，Web 才会从当前 locale 解析这两个字段；同名的 `user` preset 仍使用其文件元数据。
...
删除会移除整个 preset 目录。已经加入其常驻组装的会话继续运行；新会话则无法再从名单中解析它。
...
间接影响，取决于此后会话据以组装的 preset；该组装向模型提供什么，由 [`dsh-agent-presets`](../../preset/agent-presets/README.md) 负责。
