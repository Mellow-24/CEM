...
Host plugin 通过 `ctx.connection.fetch.handle(path, handler, options)` 注册 `/api/` 下的精确路径。该 route 共享 Connection 的 Host/Origin 信任检查，即使 `trustedHosts` 允许外层 carrier 也可要求 loopback，并直接返回 Fetch `Response`，因此其 body 可在 bridge backpressure 下流式传输。可选 route-local `maxRequestBodyBytes` 在 Node bridge 读取 request body 前与 carrier 上限取较小值；无效或重复路径会使注册失败，调用 fiber 拥有 disposal。
...
- **`/api` 桥把每个请求体整体缓冲在内存里**：`maxRequestBodyBytes`（默认 160 MiB，按默认 100 MiB 图片总量上限经 base64 膨胀加信封余量得出）是通用单请求驻留内存上界；精确 raw route 可在缓冲前降低自身上限，request-body streaming 仍需要另一条 carrier 路径。
