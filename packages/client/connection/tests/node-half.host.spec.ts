...
describe('connection node half', () => {
  ...
  it('carries exact raw Fetch responses while enforcing their loopback authority', async () => {
    const ctx = new Context()
    const routes: WebRoute[] = []
    ctx.provide('webServer', fakeHttpServer(routes, []) as WebServer)
    ctx.provide('apiProxy', {} as unknown as ApiProxy)
    ...
    const connection = ctx.get('connection') as HostConnectionHandle
    const seen: string[] = []
    const remove = connection.fetch.handle('/api/speech/sample', async (request) => {
    ...
    }, { authority: 'loopback', maxRequestBodyBytes: 5 })
    expect(() => connection.fetch.handle('/api/speech/sample', async () => new Response(), {
      authority: 'loopback',
    })).toThrow('already registered')
    expect(() => connection.fetch.handle('/outside/speech', async () => new Response(), {
      authority: 'loopback',
    })).toThrow('invalid raw Fetch path')
    ...
    }, '/api/speech/sample', 'voice'), denied.response)
    ...
    await route.handler(fakeRawPost({
      host: '127.0.0.1:3080',
      'content-type': 'audio/webm',
    }, '/api/speech/sample', 'voices'), oversized.response)
    ...
    await route.handler(fakeRawPost({
      host: '127.0.0.1:3080',
      'content-type': 'audio/webm',
    }, '/api/speech/sample', 'voice'), accepted.response)
    expect(accepted.state).toMatchObject({ status: 200, body: 'one-two' })
    expect(seen).toEqual(['voice'])

    await remove()
    const withdrawn = fakeResponse()
    await route.handler(fakeRawPost({ host: '127.0.0.1:3080' }, '/api/speech/sample', ''), withdrawn.response)
    expect(withdrawn.state.status).toBe(415)
    await fiber.dispose()
  })
  ...
  it('dispatches claimed /api endpoints before the API Proxy fallback and withdraws the claim', async () => {
    ...
    const remove = connection.rpc.intercept(
      '/api',
      endpoint => endpoint === 'goals/create' || endpoint === 'customerServiceAdmin/overview',
    ...
    // Remote endpoints normally bypass the API Proxy fallback, so the
    // configuration-plane fence must run before the shared interceptor picks
    // its claimed endpoint.
    const privilegedRemote = fakeResponse()
    await route.handler(fakePost({ host: 'harness.example' }, '/api/customerServiceAdmin/overview', {
      ...request,
      rpcId: RpcId('rpc-admin'),
      method: 'customerServiceAdmin/overview',
    }), privilegedRemote.response)
    expect(privilegedRemote.state).toMatchObject({ status: 403, body: 'forbidden' })
