 ...
 * browser cannot. Customer-service administration also exposes corpus
 * inventory, evaluation cases, and a staging write, so its entire namespace
 * remains local until the Web carrier has authentication.
 *
...
const PRIVILEGED_METHODS = new Set([
  ...
  'customerServiceAdmin/inspectConversation',
  'customerServiceAdmin/startQuality',
  'customerServiceAdmin/listQuality',
  'customerServiceAdmin/getQuality',
  'customerServiceAdmin/reviewQuality',
])
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  const route: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      const routeLimit = connection.requestBodyLimit(pathname)
      await bridge(
        req,
        res,
        fetchHandler,
        routeLimit === undefined ? maxRequestBodyBytes : Math.min(maxRequestBodyBytes, routeLimit),
      )
    },
  }
