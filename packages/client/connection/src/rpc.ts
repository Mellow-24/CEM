...
/** Handler for one exact raw Fetch route carried by Connection. */
export type ConnectionFetchHandler = (request: Request) => Promise<Response>

/** Registration policy for one raw Fetch route. */
export interface ConnectionFetchHandlerOptions extends ConnectionRpcHandlerOptions {
  /** Optional route-local cap applied by the Node bridge before it buffers the request body. */
  readonly maxRequestBodyBytes?: number
}

/** Host registry for exact, no-envelope Fetch routes under `/api`. */
export interface HostConnectionFetch {
  /**
   * Register one exact raw Fetch route and its trust policy.
   * @param path - exact absolute path below `/api/`.
   * @param handler - request handler owning the complete Fetch response, including a streaming body.
   * @param options - route trust policy.
...
export interface HostConnectionHandle {
  ...
  /** Exact no-envelope Fetch routes sharing Connection's browser trust checks and HTTP bridge. */
