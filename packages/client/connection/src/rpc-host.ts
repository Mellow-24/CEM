...
interface ConnectionFetchRoute {
  readonly handler: ConnectionFetchHandler
  readonly options: ConnectionFetchHandlerOptions
}
...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  private readonly fetchRoutes = new Map<string, ConnectionFetchRoute>()
  ...
  /** Exact no-envelope Fetch registrations scoped to the calling Context. */
  get fetch(): HostConnectionFetch {
    const owner = this.ctx
    return {
      handle: (path, handler, options) => this.registerFetch(owner, path, handler, options),
    }
  }
  ...
  createSharedFetchHandler(
    ...
    return {
      fetch: (request) => {
        ...
        const fetchRoute = this.fetchRoutes.get(pathname)
        if (fetchRoute !== undefined) {
          if (fetchRoute.options.authority === 'loopback' && !isTrustedApiRequest(request, [])) {
            return Promise.resolve(new Response('forbidden', { status: 403 }))
          }
          return fetchRoute.handler(request)
        }
  ...
  /**
   * Read the route-local request-body cap before buffering begins.
   * @param pathname - exact request pathname.
   * @returns the registered cap, or `undefined` for the carrier default.
   */
  requestBodyLimit(pathname: string): number | undefined {
    return this.fetchRoutes.get(pathname)?.options.maxRequestBodyBytes
  }

  private registerFetch(
    owner: Context,
    path: string,
    handler: ConnectionFetchHandler,
    options: ConnectionFetchHandlerOptions,
  ): () => Promise<void> {
    assertFetchPath(path)
    if (options.maxRequestBodyBytes !== undefined
      && (!Number.isSafeInteger(options.maxRequestBodyBytes) || options.maxRequestBodyBytes < 1)) {
      throw new Error('connection: raw Fetch maxRequestBodyBytes must be a positive safe integer')
    }
    return owner.effect(() => {
      if (this.fetchRoutes.has(path)) {
        throw new Error(`connection: raw Fetch route ${JSON.stringify(path)} is already registered`)
      }
      this.fetchRoutes.set(path, { handler, options })
      return () => { this.fetchRoutes.delete(path) }
    }, `client-connection: ${path} raw Fetch route`)
  }
...
function assertFetchPath(path: string): void {
  if (!path.startsWith(`${API_PATH}/`) || endpointFromPath(API_PATH, path) === undefined) {
    throw new Error(`connection: invalid raw Fetch path ${JSON.stringify(path)}`)
  }
}
