...
显式 `html[data-dsh-mobile]` 入口使用移动端 hero、可换行输入栏控件和较小的侧边留白，切换会话时不自动聚焦 textarea。空白会话也显示编辑器 dock 反馈，使录音与权限错误保持可见。会话提交、流式输出、审批和 agent（智能体）控件与桌面端使用相同的 slot 和运行时。
