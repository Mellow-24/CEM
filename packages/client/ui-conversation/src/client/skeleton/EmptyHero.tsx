...
/**
 * The jade backdrop ellipse. Rendered by the hero
  ...
  return (
    <svg className={className} viewBox="0 0 1051 468" fill="none" aria-hidden="true">
      ...
      <g filter={`url(#${glowFilterId})`}>
        <ellipse cx="525.5" cy="234" rx="425.5" ry="134" fill="var(--dsy-jade)" fillOpacity="0.12" />
      </g>
    </svg>
  )
}
...
export function HeroShell({ t, children }: HeroShellProps) {
  if (document.documentElement.hasAttribute('data-dsh-mobile')) {
    return <div className={css.mobileHero}>
      <div className={css.mobileOrb} aria-hidden="true"><BrandMark size={52} /></div>
      <h1>有什么想和我聊聊？</h1>
      <p>对话记录自动保存，随时回来继续。</p>
      {children}
    </div>
    ...
    <div className={css.root}>
      <div className={css.stack}>
        <div className={css.headline}>
          {/* Brand mark leading the headline, gap 10. */}
          <span className={css.fishHitbox}>
            <BrandMark size={30} className={css.fish} />
          </span>
