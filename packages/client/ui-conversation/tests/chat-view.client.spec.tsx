...
const assistant = (seq: number, text: string, turn = 1): AssistantMessageNode => ({
  kind: 'assistant', seq, messageId: `assistant-${String(seq)}` as never,
  time: seq * 1_000, turn, step: 1, blocks: [{ kind: 'text', text }],
})
...
function makeHarness(init?: Partial<ConversationSnapshot>) {
  ...
  const assistantActionOwners: { messageId: string; messageSeq: number }[] = []
  const renderTurnTailSlot = ((key: string, owner: object) => {
    if (key === 'conversation.chat.assistant-actions') {
      assistantActionOwners.push(owner as { messageId: string; messageSeq: number })
    }
    return null
  }) as unknown as
  ...
  return {
    ...
    chatScroll, forkAt, setSelection, toolOwners, assistantActionOwners,
  }
}
...
describe('Chat node rendering', () => {
  ...
  it('passes finalized assistant identity and durable seq to action entries', () => {
    const h = makeHarness({ nodes: [assistant(7, 'done')], turnEnds: new Map([[1, 8]]) })
    render(<h.ChatView {...h.props} />)
    expect(h.assistantActionOwners).toEqual([{ messageId: 'assistant-7', messageSeq: 7 }])
  })
