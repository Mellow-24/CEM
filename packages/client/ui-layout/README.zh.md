...
带有 `html[data-dsh-mobile]` 标记的文档选择 MobileFrame 而非 AppFrame。底部导航在常驻的对话、侧边栏和详情 slot 之间切换；选择或创建会话后返回对话。它保留共享运行时和草稿，根据可视 viewport 适应软键盘，不改变桌面入口选择。参见[独立移动入口决策](../../../.agents/notes/implemented/architecture/2026-09-11-independent-mobile-web-entry.md)。

`/client` 导出表层包含插件主体（`apply`／`inject`）、`LayoutController` 和四个 owner-share 接口。两个外壳、面板 store 与让步求解器仍属于包内部。
