...
Documents marked `html[data-dsh-mobile]` select MobileFrame instead of AppFrame. Its bottom navigation switches between the resident conversation, sidebar, and details slots; choosing or creating a Session returns to chat. It preserves the shared runtime and drafts, sizes to the visual viewport for the software keyboard, and does not change desktop entry selection. See the [independent mobile entry decision](../../../.agents/notes/implemented/architecture/2026-09-11-independent-mobile-web-entry.md).

The `/client` exports are the plugin body (`apply`/`inject`), `LayoutController`, and the four owner-share interfaces. Both frames, the panel store, and the concession solver remain package-internal.
