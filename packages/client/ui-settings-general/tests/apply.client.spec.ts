...
import type { SettingsChromeInjected } from '../src/client/chrome.tsx'
import { GeneralSection } from '../src/client/GeneralSection.tsx'
import { SettingsDocumentAction } from '../src/client/SettingsDocumentAction.tsx'
import type { SettingsDocumentActionInjected } from '../src/client/SettingsDocumentAction.tsx'
import { CUSTOMER_SERVICE_OVERVIEW_SECTION_ID } from '../src/client/shell-contract.ts'
...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    expect(resolveSlotLabel(entry.options.label)).toBe('通用設定')
  ...
  it('registers the settings dictionaries and frees the seats on teardown', async () => {
    const b = await bench()
    declare(b.slots)
    const fiber = b.ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    expect(b.locale.bind('settings')('title')).toBe('設定')
    expect(b.locale.bind('settings')('management.title')).toBe('澳門電力智能客服營運管理台')
    b.locale.setLocale('en')
    expect(b.locale.bind('settings')('close')).toBe('Close')
    expect(b.locale.bind('settings')('management.trigger')).toBe('Operations')
    b.locale.setLocale('pt')
    expect(b.locale.bind('settings')('management.trigger')).toBe('Operações')
    expect(b.locale.bind('settings')('management.title')).toBe('Consola de Operações do Atendimento Inteligente da CEM')
    expect(b.locale.bind('settings')('management.maintenance')).toBe('Manutenção técnica')
    ...
    expect(() => b.locale.register('settings', 'pt', {})).not.toThrow()
  })
  ...
  it('the nav label thunk follows the active locale without re-registration', async () => {
    ...
    b.locale.setLocale('zh')
    expect(resolveSlotLabel(generalEntry(b.slots)!.options.label)).toBe('通用設定')
  })

  it('activates operations chrome while the customer-service overview is registered', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const sources = (['settings.trigger', 'settings.header'] as const).map((name) => {
      const entry = b.slots.entries(name)[0]!
      return (entry.inject as unknown as () => SettingsChromeInjected)().hooks.managementMode
    })
    expect(sources.map(source => source.getSnapshot())).toEqual([false, false])
    const listeners = sources.map(() => vi.fn())
    const offs = sources.map((source, index) => source.subscribe(listeners[index]!))
    const remove = b.slots.register({
      name: 'settings.section',
      id: CUSTOMER_SERVICE_OVERVIEW_SECTION_ID,
      order: 40,
      label: '營運總覽',
    } as never, () => null)
    await Promise.resolve()
    expect(sources.map(source => source.getSnapshot())).toEqual([true, true])
