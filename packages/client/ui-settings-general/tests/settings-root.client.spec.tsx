...
} from '../src/client/shell-contract.ts'
...
function mount({
  ...
  const props: SettingsRootComponentProps = {
    maintenanceLabel: () => 'Developer maintenance',
    useSessions,
...
describe('SettingsRoot trigger', () => {
  ...
  it('opens customer-service operations as a full-screen page', () => {
    mount({ rows: [{ id: CUSTOMER_SERVICE_OVERVIEW_SECTION_ID, order: 40, label: 'Overview' }] })
    openPanel()
    expect(screen.queryByRole('dialog')).toBeNull()
    expect(screen.getByRole('region', { name: 'Settings Title' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Settings' }).hasAttribute('aria-haspopup')).toBe(false)
  })
})
...
describe('SettingsPanel navigation', () => {
  ...
  it('gives every section a nav glyph, distinct for the ids the shell knows', () => {
    mount({
      rows: [
        ...
        { id: 'customer-service-overview', order: 40, label: 'Overview' },
        { id: 'customer-service-knowledge', order: 41, label: 'Knowledge' },
        { id: 'customer-service-sessions', order: 42, label: 'Sessions' },
        { id: 'customer-service-evaluation', order: 43, label: 'Evaluation' },
    ...
    const labels = [
      'General', 'Models', 'Agent presets', 'Plugins',
      'Overview', 'Knowledge', 'Sessions', 'Evaluation', 'Contributed',
    ]
    const nav = screen.getByRole('navigation', { name: 'Settings Title' })
    expect(within(nav).getByText('Developer maintenance', { exact: true })).toBeTruthy()
    expect(within(nav).getAllByRole('button')).toHaveLength(labels.length)
    const glyphs = labels
    ...
    // Built-in and customer-service sections receive distinct glyphs; an
    // independently contributed id still receives the generic settings icon.
  ...
  it('opens on the customer-service overview when that section is registered', () => {
    mount({
      rows: [
        { id: 'general', order: 0, label: 'General' },
        { id: CUSTOMER_SERVICE_OVERVIEW_SECTION_ID, order: 40, label: 'Overview' },
        { id: 'customer-service-sessions', order: 42, label: 'Sessions' },
      ],
    })
    openPanel()
    expect(screen.getByRole('button', { name: 'Overview' }).getAttribute('aria-current')).toBe('page')
    expect(screen.getByTestId(`section-${CUSTOMER_SERVICE_OVERVIEW_SECTION_ID}`)).toBeTruthy()
