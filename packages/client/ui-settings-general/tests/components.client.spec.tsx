...
const managementHook = (value: boolean): TriggerContentProps['useManagementMode'] =>
  ((select: (mode: boolean) => unknown) => select(value)) as TriggerContentProps['useManagementMode']

describe('chrome content', () => {
  it('TriggerContent renders the icon with the label in the wide column', () => {
    const { container } = render(<TriggerContent {...kit} wide useManagementMode={managementHook(false)} t={t} />)
  ...
  it('TriggerContent drops the label in the rail state', () => {
    const { container } = render(
      <TriggerContent {...kit} wide={false} useManagementMode={managementHook(false)} t={t} />,
    )
    expect(container.querySelector('svg')).toBeTruthy()
  ...
  it('HeaderContent and CloseLabel render their translated text', () => {
    render(<HeaderContent {...kit} useManagementMode={managementHook(false)} t={t} />)
    render(<CloseLabel {...kit} t={t} />)
    expect(screen.getByText('Settings')).toBeTruthy()
    expect(screen.getByText('Close')).toBeTruthy()
  })

  it('uses customer-service operations copy when the overview section is present', () => {
    render(<TriggerContent {...kit} wide useManagementMode={managementHook(true)} t={t} />)
    render(<HeaderContent {...kit} useManagementMode={managementHook(true)} t={t} />)
    expect(screen.getByText('Operations')).toBeTruthy()
    expect(screen.getByText('Macau Power Intelligent Customer Service Operations Console')).toBeTruthy()
  })
})
