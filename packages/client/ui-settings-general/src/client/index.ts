 ...
 * `sidebar.settings` occupant — operations-console chrome, section navigation, and the
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en, pt }), 'ui-settings-general: dictionaries')
  ...
  const managementMode = {
    getSnapshot: () => ctx.slots.entries('settings.section')
      .some(entry => entry.options.id === CUSTOMER_SERVICE_OVERVIEW_SECTION_ID),
    subscribe: (listener: () => void) => ctx.slots.subscribe('settings.section', listener),
  }
  const chromeInjected = (): SettingsChromeInjected => ({ hooks: { managementMode } })
  ...
  const shellInjected = (): SettingsRootInjected => ({
    maintenanceLabel: () => t('management.maintenance'),
    hooks: {
      sections: {
        getSnapshot: () => {
