 ...
 * extensions ship as octet-stream, non-GET/HEAD is 405. The index and mobile
 * application documents run through the webserver's registered index taps (boot-manifest
 ...
 * @param renderIndex - transforms a built application document; omitted path selects index.html.
 */
export async function serveStatic(
  pathname: string, res: ServerResponse, distRoot: string, distIndex: string,
  renderIndex: (documentPath?: string) => Promise<string>,
): Promise<void> {
  ...
  if (target === join(distRoot, 'mobile.html')) {
    const body = await renderIndex(target)
...
export function apply(ctx: Context, config: Config): void {
  ...
  const renderIndex = async (documentPath = distIndex): Promise<string> =>
    ctx.webServer.applyIndexTaps(await readFile(documentPath, 'utf8'))
