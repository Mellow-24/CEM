...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(join(dist, 'mobile.html'), '<head></head><body>mobile</body>')
  await writeFile(join(dist, 'reference.html'), '<head></head><body>reference</body>')
...
describe('real Loader composition', () => {
  it('serves the dist with SPA fallback, taps, traversal rejection, and method gating', { timeout: 60_000 }, async () => {
    ...
    const mobile = await request(port, '/mobile.html')
    expect(mobile).toMatchObject({ status: 200, type: 'text/html; charset=utf-8' })
    expect(mobile.body).toContain('__T__')
    expect(mobile.body).toContain('mobile')
    expect(mobile.body).not.toContain('shell')
    expect((await request(port, '/reference.html')).body).not.toContain('__T__')
    expect((await request(port, '/..%2fmobile.html')).status).toBe(403)
    expect((await request(port, '/mobile.html', { method: 'POST' })).status).toBe(405)
    untap()
