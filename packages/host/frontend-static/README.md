...
The built `/mobile.html` is a second application document: it also receives index taps and is served with `Cache-Control: no-store`. Other HTML assets are served unchanged. This route does not change the webserver's bind address or authorization policy.
