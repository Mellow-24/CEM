...
/**
 * OpenAI-completions compatibility switches, set on the route (its models'
 * default) or per model (winning over the route). Only switches needed to
 * describe reasoning dispatch and message roles are offered; the rest of
 * pi-ai's compat surface keeps its baseURL-derived auto-detection. pi-ai types
 * these fields only on `OpenAICompletionsCompat`, so resolution rejects a
...
export interface PiAiCompatProfile {
  ...
  /** Whether the endpoint accepts the OpenAI `developer` role; absent keeps the catalog entry's, then pi-ai's baseURL-derived guess. */
...
/**
 * Resolve one model's compat block from the profile's completion switches.
 *
...
function resolveModelCompat(
  ...
  if (api !== 'openai-completions') {
    if (entry.compat?.thinkingFormat !== undefined || entry.compat?.supportsReasoningEffort !== undefined
      ...
      invalid(provider, `model "${entry.id}" sets openai-completions compat switches, but its api is "${api}";`
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  if (routeCompatDefined && !models.some(model => model.api === 'openai-completions')) {
    invalid(provider, 'sets openai-completions compat switches, but no model on the route speaks that protocol;'
      + ' thinkingFormat, supportsReasoningEffort, and supportsDeveloperRole exist only there')
  }
