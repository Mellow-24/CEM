...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'speechSynthesis',
    summary: 'Scoped synthesis provider registry and streaming runtime.',
    description: 'Scoped synthesis provider registry and streaming runtime.',
    methods: [
      {
        signature: 'registerProvider(provider: SpeechSynthesisProvider): () => void',
        description: 'Register a provider in the calling context\'s scope layer.',
        parameters: [{ name: 'provider', description: 'borrowed provider and its public profile metadata.' }],
        returns: 'exact effect disposer removing this registration.',
      },
      {
        signature: 'resolve(agent: Agent, profile?: string): ResolvedSpeechSynthesis',
        description: 'Resolve one profile visible to an exact live agent.',
        parameters: [{ name: 'agent', description: 'exact live agent whose scope chain supplies permission.' }, { name: 'profile', description: 'explicit profile, or omission when exactly one is visible.' }],
        returns: 'registration-bound operation and detached profile metadata.',
      },
      {
        signature: 'profile(agent: Agent, profile?: string): SpeechSynthesisProfile',
        description: 'Read public metadata for one profile visible to an exact live agent.',
        parameters: [{ name: 'agent', description: 'exact live agent whose scope chain supplies permission.' }, { name: 'profile', description: 'explicit profile, or omission when exactly one is visible.' }],
  ...
  },
  {
    ...
    summary: 'Scoped transcription provider registry and execution runtime.',
    description: 'Scoped transcription provider registry and execution runtime.',
    methods: [
      {
        signature: 'registerProvider(provider: SpeechTranscriptionProvider): () => void',
        description: 'Register a provider in the calling context\'s scope layer.',
        parameters: [{ name: 'provider', description: 'borrowed provider and its public profile metadata.' }],
        returns: 'exact effect disposer removing this registration.',
      },
      {
        signature: 'resolve(agent: Agent, profile?: string): ResolvedSpeechTranscription',
        description: 'Resolve one profile visible to an exact live agent.',
        parameters: [{ name: 'agent', description: 'exact live agent whose scope chain supplies permission.' }, { name: 'profile', description: 'explicit profile, or omission when exactly one is visible.' }],
        returns: 'registration-bound operation and detached profile metadata.',
      },
      {
        signature: 'profile(agent: Agent, profile?: string): SpeechTranscriptionProfile',
        description: 'Read public metadata for one profile visible to an exact live agent.',
        parameters: [{ name: 'agent', description: 'exact live agent whose scope chain supplies permission.' }, { name: 'profile', description: 'explicit profile, or omission when exactly one is visible.' }],
        returns: 'detached immutable profile metadata.',
      },
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'agent/post-response',
    mode: 'waterfall',
    signature: '\'agent/post-response\'(this: Scoped<Agent>, payload: { agent: Agent; turn: number; step: number; message: AssistantMessage; signal: AbortSignal }, next: () => Promise<PostResponseDecision>): Promise<PostResponseDecision>',
    summary: 'Accept a buffered completed response or discard it and retry the same request.',
    description: 'Accept a buffered completed response or discard it and retry the same request. A listener that returns `{ kind: \'retry\' }` owns recovery and does not call `next()`; all other listeners call `next()` to delegate. The retry reassembles the system prompt but leaves the durable user and tool history unchanged. This event is never dispatched for a streamed, max-token, failed, aborted, or tool-call response.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'PostResponseDecision',
    declaration: 'export type PostResponseDecision = {\n    kind: \'accept\';\n} | {\n    kind: \'retry\';\n};',
  },
  {
  ...
  {
    name: 'SpeechAudioMetadata',
    declaration: 'export interface SpeechAudioMetadata {\n    readonly mediaType: string;\n    readonly contentLength?: number;\n    readonly sampleRateHz?: number;\n    readonly channels?: number;\n}',
  },
  {
    name: 'SpeechRealtimeEvent',
    declaration: 'export type SpeechRealtimeEvent = {\n    readonly type: \'speech-start\';\n} | {\n    readonly type: \'partial\' | \'final\';\n    readonly text: string;\n} | {\n    readonly type: \'error\';\n    readonly message: string;\n};',
  },
  {
    name: 'SpeechRealtimeInput',
    declaration: 'export interface SpeechRealtimeInput {\n    send(pcm: Uint8Array): Promise<void>;\n    close(): Promise<void>;\n}',
  },
  {
    name: 'SpeechSynthesisInput',
    declaration: 'export interface SpeechSynthesisInput {\n    readonly text: string;\n    readonly language?: string;\n}',
  },
  {
    name: 'SpeechSynthesisOutput',
    declaration: 'export interface SpeechSynthesisOutput {\n    readonly metadata: SpeechAudioMetadata;\n    readonly chunks: AsyncIterable<Uint8Array>;\n}',
  },
  {
    name: 'SpeechSynthesisProvider',
    declaration: 'export interface SpeechSynthesisProvider extends SpeechSynthesisProfile {\n    synthesize(input: SpeechSynthesisInput, signal: AbortSignal): Promise<SpeechSynthesisOutput>;\n}',
  },
  {
  ...
  {
    name: 'SpeechTranscriptionInput',
    declaration: 'export interface SpeechTranscriptionInput {\n    readonly data: Uint8Array;\n    readonly mediaType: string;\n}',
  },
  {
    name: 'SpeechTranscriptionProvider',
    declaration: 'export interface SpeechTranscriptionProvider extends SpeechTranscriptionProfile {\n    openRealtime?(event: (event: SpeechRealtimeEvent) => void, signal: AbortSignal): Promise<SpeechRealtimeInput>;\n    transcribe(input: SpeechTranscriptionInput, signal: AbortSignal): Promise<SpeechTranscript>;\n}',
  },
  {
    name: 'SpillLocator',
