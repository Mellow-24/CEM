...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'register<N extends keyof LocaleNamespaceMap & string>(ns: N, dicts: DeclaredLocaleDictionaries<N>): () => void',
        description: 'Register a declared namespace\'s dictionaries, all locales in one call — the typed form: each dictionary is checked against the namespace\'s LocaleNamespaceMap key union. The complete zh fallback is required; other shipped locales may be added per namespace as their product surface is translated. Duplicate (ns, locale) throws (single occupant; a namespace\'s texts have one owner). Registration bumps the revision so mounted outlets pick up late-arriving dictionaries.',
        parameters: [{ name: 'ns', description: 'a namespace merged into LocaleNamespaceMap.' }, { name: 'dicts', description: 'fallback dictionary plus translated locale dictionaries.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'DeclaredLocaleDictionaries',
    declaration: 'export type DeclaredLocaleDictionaries<N extends keyof LocaleNamespaceMap & string> = {\n    zh: LocaleDictOf<N>;\n} & Partial<Record<Exclude<LocaleId, \'zh\'>, LocaleDictOf<N>>>;',
  },
  {
    name: 'EntryKeyOf',
