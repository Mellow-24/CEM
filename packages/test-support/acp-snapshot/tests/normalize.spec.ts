...
describe('normalizeSessionLog', () => {
  ...
  it('tokenizes volatile command ids while preserving lifecycle correlation', () => {
    const run = JSON.stringify({
      type: 'command/run', seq: 1, time: 5,
      data: { commandId: 'cmd-random-a-1', name: 'response-language' },
    })
    const done = JSON.stringify({
      type: 'command/done', seq: 2, time: 6,
      data: { commandId: 'cmd-random-a-1', kind: 'success' },
    })
    const second = JSON.stringify({
      type: 'command/run', seq: 3, time: 7,
      data: { commandId: 'cmd-random-b-2', name: 'other' },
    })

    const out = normalizeSessionLog(`${header({})}\n${run}\n${done}\n${second}\n`, ctx)

    expect(out.match(/\{\{commandId:1\}\}/gu)).toHaveLength(2)
    expect(out).toContain('{{commandId:2}}')
    expect(out).not.toContain('cmd-random')
  })
