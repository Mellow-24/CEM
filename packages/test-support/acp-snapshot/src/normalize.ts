 ...
 * zeroed/scrubbed, command lifecycle ids are replaced by correlated sequence
 * tokens, all volatile strings scrubbed, and `seq` is LEFT INTACT
...
export function normalizeSessionLog(
  ...
  const records = lines.map((line) => {
    ...
    if (data !== null && typeof data === 'object' && !Array.isArray(data)) {
      ...
      const commandId = dataRecord.commandId
      if (typeof commandId === 'string') {
        let stable = commandIds.get(commandId)
        if (stable === undefined) {
          stable = commandIds.size + 1
          commandIds.set(commandId, stable)
        }
        dataRecord.commandId = `{{commandId:${String(stable)}}}`
      }
    }
