...
The provider places the resolved credential in the server-side `Authorization` header. The endpoint URL never receives the token. Each operation uses a fresh opaque client and request id, requires `start` before binary audio and `end` after at least one frame, and rejects unrelated or malformed events. The caller signal and `timeoutMs` terminate the socket; `firstAudioTimeoutMs` fails a generation that acknowledges `start` but emits no audio. Provider capacity responses with code 429 are reported as `PROVIDER_BUSY`. `maxEventBytes` bounds each WebSocket frame; `maxOutputBytes` bounds received and queued audio.
...
- **One socket per sentence** — ordered sentence playback opens a new provider connection for each admitted sentence; persistent connection reuse needs an ownership and teardown policy across HTTP requests.
